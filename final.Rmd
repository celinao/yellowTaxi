...
![Figure 1: Passenger Trends over time](TaxiPassengersLinePlot.png)   

We created a time series plot of the number of taxi passengers in each zone where each zone represents a neighborhood of New York City. Our plot shows that some zones have higher passenger counts on average, and that the overall number of passengers across taxi zones tends to decrease over time. It also appears that the number of passengers is lower on average during the summer months (June - August), which could indicate that fewer people want to take taxis in warmer weather. Finally, there is a noticeable dip in passenger count in January 2019 due to a severe winter storm.  


![Figure 2: Passenger Heatmap](CombinedHeatMapJanAug.png)
...
Additionally, we chose January and August because these were months of interest from Figure 1. As you can see in January 2019, there is a slightly lighter red than the other years, indicating less passengers because of the winter storm that year. We chose to visualize August in this report because it is a summer month, and we wanted to investigate if we could visualize a difference between the summer and winter months. Plots of every year and month are available via a Shiny application by running "createGraphs.Rmd" as found on our github.
